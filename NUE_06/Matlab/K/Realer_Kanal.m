...
% @ ScopeHandle    - Gerätehandle für das PicoScope
% @ OutputData     - (kxm) Bitstreams der Ausgabe. Die Zeilen der Matrix stehen
%                    für die einzelnen Kanäle (k={1,2,3,4})
% @ FilterParmeter - Objekt mit zusätzlichen Daten für die Filterung der
%                    aufgenommenen Samples
% @ DEBUGParameter - Objekt mit diversen Debugparamter (ist für Probleme im
%                    Versuch gedacht. Nicht ändern.)
% @ Values         - Rückgabe
...
%% Ready
if status==1
    PCM_Decod(KanalParameter.NoiseFactor);
    ...
    TInt=StartTriggerBlock(KanalParameter.ScopeVersion,Channels,RangeIndex,SampleRate,11000,KanalParameter.ScopeHandle,1,0,1,0);
    ParallelOUT(0,1);
...
% Diese Pointer (speziell pBuffer, pBuffer2 und overflow) MÜSSEN hier
% definiert werden. Sie werden zwar in dieser Funktion nicht benötigt, aber
% Matlab (2007a) stürzt ab, wenn ich sie woanders hin schreibe. Auch die
% Änderung der Reihenfolge führt zum Absturz.
